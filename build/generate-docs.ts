...
We recommend looking at the [examples](../examples/index.md) as they will help you the most to start with Mapmetrics.
...
async function generateExamplesFolder() {
    ...
    const mapmetricsUnpkg = `https://unpkg.com/mapmetrics-gl@${packageJson.version}/`;
    const indexArray = [] as HtmlDoc[];
    for (const file of files) {
        const htmlFile = path.join(examplesFolder, file);
        let htmlContent = fs.readFileSync(htmlFile, 'utf-8');
        htmlContent = htmlContent.replace(/\.\.\/\.\.\//g, mapmetricsUnpkg);
...
async function generatePluginsPage() {
    /**
     * It extract some sections from Awesome Mapmetrics README.md so we can integrate it into our plugins page
     *
     * ```
     *    header
     *    <!-- [SOME-ID]:BEGIN -->
     ...
     * ```
     */
    const awesomeReadmeUrl = 'https://raw.githubusercontent.com/mapmetrics/awesome-mapmetrics/main/README.md';
...
function updateMapmetricsVersionForUNPKG() {
    ...
    indexContent = indexContent.replace(/unpkg\.com\/mapmetrics-gl@\^(\d+\.\d+\.\d+)/g, `unpkg.com/mapmetrics-gl@^${packageJson.version}`);
...
updateMapmetricsVersionForUNPKG();
