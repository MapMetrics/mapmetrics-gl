...
async function createImage(exampleName) {
    ...
    if (useLocalhost) {
        ...
        await page.setContent(html.replaceAll('../../dist', `https://unpkg.com/mapmetrics-gl@${packageJson.version}/dist`));
    }

    // Wait for map to load, then wait two more seconds for images, etc. to load.
    try {
        // @ts-ignore
        await page.evaluate(() => document.querySelector('.mapmetricsgl-ctrl-attrib').style.display = 'none');
