...
try {
    ...
    await webPage.waitForFunction(() => (window as any).mapmetricsglBenchmarkFinished);
    const allNames = await webPage.evaluate(() => Object.keys((window as any).mapmetricsglBenchmarks));
    const versions = await webPage.evaluate((name) => Object.keys((window as any).mapmetricsglBenchmarks[name]), allNames[0]);
    ...
    for (const name of toRun) {
        ...
        await webPage.waitForFunction(
            () => (window as any).mapmetricsglBenchmarkFinished,
            {
                polling: 200,
                timeout: 0
            }
        );
        const results = await webPage.evaluate((name) => (window as any).mapmetricsglBenchmarkResults[name], name);
