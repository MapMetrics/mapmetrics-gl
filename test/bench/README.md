...
```bash
MAPMETRICS_STYLES={YOUR STYLES HERE} npm run start-bench
```
Note: `MAPMETRICS_STYLES` takes a comma-separated list of up to 3 Mapmetrics styles provided as a style URL or file system path (e.g. `path/to/style-a.json,path/to/style-b.json`)
...
Build minimized production mapmetrics-gl-js with `npm run build-prod`.
