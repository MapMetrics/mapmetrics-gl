...
This part of the docs is dedicated to the migration from `openlayers` to `mapmetrics-gl`.

## Setting Up Mapmetrics

Install Mapmetrics GL JS and replace OpenLayers with Mapmetrics in your project:
...
map.on("load", function () {
    new mapmetricsgl.Marker({ color: "red" }).setLngLat([0, 0]).addTo(map);
});
```
