...
import type {MapmetricsEvent, MapMouseEvent} from './events';
...
export class Popup extends Evented {
     ...
     *
     * @param map - The Mapmetrics GL JS map to add the popup to.
     * @example
     * ```ts
     * new Popup()
     *   .setLngLat([0, 0])
    ...
    addTo(map: Map): this {
        ...
        if (this._trackPointer) {
            ...
            if (this._container) {
                this._container.classList.add('mapmetricsgl-popup-track-pointer');
            }
            this._map._canvasContainer.classList.add('mapmetricsgl-track-pointer');
        } else {
    ...
    remove = (): this => {
        ...
        if (this._map) {
            ...
            this._map._canvasContainer.classList.remove('mapmetricsgl-track-pointer');
            delete this._map;
    ...
    setLngLat(lnglat: LngLatLike): this {
        ...
        if (this._map) {
            ...
            this._map.off('mousemove', this._update);
            if (this._container) {
                this._container.classList.remove('mapmetricsgl-popup-track-pointer');
            }
            this._map._canvasContainer.classList.remove('mapmetricsgl-track-pointer');
        }
    ...
    setDOMContent(htmlNode: Node): this {
        if (this._content) {
            ...
            this._content = DOM.create('div', 'mapmetricsgl-popup-content', this._container);
        }
    ...
    _createCloseButton() {
        if (this.options.closeButton) {
            this._closeButton = DOM.create('button', 'mapmetricsgl-popup-close-button', this._content);
    ...
    _update = (event?: MapmetricsEvent | MapMouseEvent) => {
        ...
        if (!this._container) {
            this._container = DOM.create('div', 'mapmetricsgl-popup', this._map.getContainer());
            this._tip       = DOM.create('div', 'mapmetricsgl-popup-tip', this._container);
