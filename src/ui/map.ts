...
export type MapOptions = {
     ...
     * !!! note
     *     Showing the logo of Mapmetrics is not required for using Mapmetrics.
     * @defaultValue compact: true, customAttribution: "Mapmetrics ...".
     */
    ...
    trackResize?: boolean;
    /**
     * The initial geographical centerpoint of the map. If `center` is not specified in the constructor options, Mapmetrics GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `[0, 0]`
     * !!! note
     *     Mapmetrics GL JS uses longitude, latitude coordinate order (as opposed to latitude, longitude) to match GeoJSON.
     * @defaultValue [0, 0]
     */
    center?: LngLatLike;
    /**
    ...
    elevation?: number;
    /**
     * The initial zoom level of the map. If `zoom` is not specified in the constructor options, Mapmetrics GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
     * @defaultValue 0
     */
    zoom?: number;
    /**
     * The initial bearing (rotation) of the map, measured in degrees counter-clockwise from north. If `bearing` is not specified in the constructor options, Mapmetrics GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
     * @defaultValue 0
     */
    bearing?: number;
    /**
    ...
    pitch?: number;
    /**
     * The initial roll angle of the map, measured in degrees counter-clockwise about the camera boresight. If `roll` is not specified in the constructor options, Mapmetrics GL JS will look for it in the map's style object. If it is not specified in the style, either, it will default to `0`.
     * @defaultValue 0
     */
    roll?: number;
    /**
    ...
    /**
     * The map's Mapmetrics style. This must be a JSON object conforming to
     * the schema described in the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/),
    ...
    /**
     * If `true`, enables the SEO/AEO layer with default settings. If an `SeoConfig` object is
     * provided, enables the SEO/AEO layer with the specified configuration. The SEO layer
     * generates structured data (JSON-LD, meta tags) for map content to improve search engine
     * and AI engine discoverability.
     * @defaultValue undefined
     */
...
export class Map extends Camera {
    ...
    /** See {@link MapOptions.showTileGrids}. Gates the whole tile-grid debug overlay. */
    _showTileGrids: boolean;
    ...
    _markers: Set<Marker>;
    /**
     * The map's {@link TileLoadingManager}, which manages tile loading states and provides
     * utilities to wait for tiles to load before completing zoom animations.
     */
    ...
    constructor(options: MapOptions) {
        ...
        // The tile-grid overlay is a DEBUG feature and is OFF unless asked for. It used to run
        // unconditionally -- `showTileGrids` was declared in MapOptions and then never read --
        // which meant every map drew grid overlays during load AND had
        // `background-pattern: grid-pulse-pattern` injected into its style by
        // `_createDefaultBackgroundPattern`, so `map.getStyle()` no longer round-tripped the
        // style the caller passed in. Mutating a customer's style is not an acceptable default.
        this._showTileGrids = resolvedOptions.showTileGrids === true;
        ...
        // LICENCE / CONTRACTUAL, NOT COSMETIC. The attribution control is mandatory: consumers may
        // customise it but cannot disable it. `attributionControl: false` is deliberately coerced
        // back to the defaults rather than honoured, which is why this is NOT upstream's
        // `if (resolvedOptions.attributionControl)`. Restoring upstream's conditional silently
        // re-enables stripping the attribution — see MAPMETRICS-FORK.md §3 item 12.
        ...
        this.on('data', (event: MapDataEvent) => {
            ...
            // Immediately, to remove grids as soon as tiles start rendering.
        ...
        // SEO layer
        if (resolvedOptions.seo) {
            const seoConfig: SeoConfig = typeof resolvedOptions.seo === 'boolean'
                ? {enabled: resolvedOptions.seo}
                : resolvedOptions.seo;
    ...
    */
     ...
     *
     * @param minPitch - The minimum pitch to set (0-180). Values greater than 60 degrees are experimental and may result in rendering issues. If you encounter any, please raise an issue with details in the Mapmetrics project.
     ...
     *
     * @param maxPitch - The maximum pitch to set (0-180). Values greater than 60 degrees are experimental and may result in rendering issues. If you encounter any, please raise an issue with details in the Mapmetrics project.
     ...
     * [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/), or a URL to such JSON.
     ...
     * Mapmetrics Style Specification's [source definition](https://maplibre.org/maplibre-style-spec/sources) or
     ...
     * A list of options for each source type is available on the Mapmetrics Style Specification's
    ...
    /**
     * Adds a [Mapmetrics style layer](https://maplibre.org/maplibre-style-spec/layers)
     * to the map's style.
     *
     * A layer defines how data from a specified source will be styled. Read more about layer types
     * and available paint and layout properties in the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/layers).
     *
     * @param layer - The layer to add,
     * conforming to either the Mapmetrics Style Specification's [layer definition](https://maplibre.org/maplibre-style-spec/layers) or,
     ...
     * The Mapmetrics Style Specification's layer definition is appropriate for most layers.
     *
     ...
     * @param filter - The filter, conforming to the Mapmetrics Style Specification's
     ...
     * Must be of a type appropriate for the property, as defined in the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/).
     ...
     * @param value - The value of the layout property. Must be of a type appropriate for the property, as defined in the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/).
     * @param options - The options object.
     * @example
     * ```ts
    ...
    * to unset glyphs.
     *
     * @param glyphsUrl - Glyph URL to set. Must conform to the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/glyphs/).
     * @param options - Options object.
     * @example
     * ```ts
     ...
     *
     * @param light - Light properties to set. Must conform to the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/light).
     * @param options - Options object.
     *
     * @example
     * ```ts
     ...
     *
     * @param sky - Sky properties to set. Must conform to the [Mapmetrics Style Specification](https://maplibre.org/maplibre-style-spec/sky/).
     * @param options - Options object.
     *
     * @example
     * ```ts
     ...
     *
     * Called from `Marker#addTo`. Nothing in the render path depends on it, so if this hook is
     * lost markers still draw perfectly and the entire `src/seo/` feature silently emits nothing.
     */
    ...
    _setupContainer() {
        ...
        container.classList.add('mapmetricsgl-map');

        const canvasContainer = this._canvasContainer = DOM.create('div', 'mapmetricsgl-canvas-container', container);
        if (this._interactive) {
            canvasContainer.classList.add('mapmetricsgl-interactive');
        }

        this._canvas = DOM.create('canvas', 'mapmetricsgl-canvas', canvasContainer);
        ...
        // Add the tile-grid container INSIDE the canvas container, so grids only ever show
        // over the map and never over user UI components.
    ...
    // ---------------------------------------------------------------------------------
    // Tile-grid debug overlay (fork feature, gated on MapOptions.showTileGrids)
    // ---------------------------------------------------------------------------------
    ...
    _addTileGrid(tile: any) {
        if (!this._showTileGrids) return;
        if (!this._tileGridContainer || !tile?.tileID) return;
    ...
    _updateTileGridPosition(tile: any, gridOverlay: HTMLElement) {
        if (!tile?.tileID || !gridOverlay) return;
    ...
    _preloadTilesAtMultipleZoomLevels() {
        if (!this._showTileGrids) return;
        if (!this.style?.tileManagers) return;
    ...
    _updateTileGridOverlays() {
        if (!this._showTileGrids) return;
        if (!this.style?.tileManagers || !this._tileGridContainer) return;
        ...
        for (const sourceCache of Object.values(this.style.tileManagers)) {
            ...
            for (const tileKey in source._tiles) {
                ...
                if (tile && (tile.texture || tile.state === 'loaded' || (tile.hasData?.()))) {
    ...
    remove() {
        ...
        this._container.classList.remove('mapmetricsgl-map');
    ...
    /**
     * @internal
     * Re-request tiles that failed, after a v2 map-session credential is adopted.
     *
     ...
     * `TileManager.reload(true)` resets the tile cache and re-requests every tile it holds, not
     ...
     *
     * THE `hasErroredTiles()` CALL IS THE GATE. Do not replace it with an unconditional
     * `reload(true)` — that is pure billed cost with no visible symptom. See
     * MAPMETRICS-FORK.md §3 item 5. (`style.sourceCaches` became `style.tileManagers` in
     * upstream #6635.)
     */
    _reloadErroredTiles() {
        ...
        for (const id in this.style.tileManagers) {
            const tileManager = this.style.tileManagers[id];
            if (!tileManager.hasErroredTiles()) continue;
            tileManager.reload(true);
        }
    }
