...
describe('CoopGesturesHandler', () => {
    ...
    test('Does not zoom on wheel if no key is down', async () => {
        ...
        map._renderTaskQueue.run();
        expect(map.getContainer().querySelector('.mapmetricsgl-cooperative-gesture-screen.mapmetricsgl-show')).toBeInstanceOf(HTMLDivElement);
        ...
        expect(map.getContainer().querySelector('.mapmetricsgl-cooperative-gesture-screen.mapmetricsgl-show')).toBeNull();
        map.remove();
    });
