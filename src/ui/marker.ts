 ...
 * ## CSS Classes
 *
 * **CSS class** `mapmetricsgl-marker-covered` is toggled on the marker element when the marker
 * is hidden behind 3D terrain or on the back of a globe.
 * Use this class to apply custom styles to covered markers.
 *
 * @example
 * ```css
 * .mapmetricsgl-marker-covered {
 *     pointer-events: none;
 *     cursor: default;
 * }
 * ```
...
export class Marker extends Evented {
     ...
     * @param map - The Mapmetrics GL JS map to add the marker to.
     * @example
     * ```ts
     * let marker = new Marker()
     *   .setLngLat([30.5, 50.5])
    ...
    addTo(map: Map): this {
        ...
        // Register marker for SEO tracking. Nothing in the render path depends on this, so if it
        // is lost markers still draw perfectly and src/seo/ silently emits nothing.
        map._addMarker(this);
    ...
    _updateOpacity(force: boolean = false) {
        ...
        if (!terrain || occluded) {
            ...
            if (this._element.style.opacity !== targetOpacity) {
                ...
                this._element.classList.toggle('mapmetricsgl-marker-covered', occluded);
            }
            return;
        }
        if (force) {
        ...
        if (markerDistance - terrainDistance < forgiveness) {
            ...
            this._element.classList.remove('mapmetricsgl-marker-covered');
            return;
        }
        ...
        this._element.classList.toggle('mapmetricsgl-marker-covered', centerIsInvisible);
    }
