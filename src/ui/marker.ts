...
export class Marker extends Evented {
    ...
    _onMapClick = (e: MapMouseEvent) => {
        ...
        if (
            ...
            (targetElement === element ||
                element.contains(targetElement as any))
        ) {
            this.togglePopup();
        }
    };
    ...
    _updateOpacity(force: boolean = false) {
        ...
        if (!terrain) {
            const occluded = this._map.transform.isLocationOccluded(
                this._lngLat
            );
            const targetOpacity = occluded
        ...
        const markerDistance = map.transform.lngLatToCameraDepth(
            this._lngLat,
            elevation
        );

        const forgiveness = 0.006;
        ...
        const centerIsInvisible =
            markerDistanceCenter - terrainDistanceCenter > forgiveness;

        if (this._popup?.isOpen() && centerIsInvisible) this._popup.remove();
        this._element.style.opacity = centerIsInvisible
            ? this._opacityWhenCovered
            : this._opacity;
    }
    ...
    _update = (e?: { type: "move" | "moveend" | "terrain" | "render" }) => {
        ...
        this._flatPos = this._pos = this._map
        ...
        DOM.setTransform(
            this._element,
            `${anchorTranslate[this._anchor]} translate(${this._pos.x}px, ${
                this._pos.y
            }px) ${pitch} ${rotation}`
        );
    ...
    _onMove = (e: MapMouseEvent | MapTouchEvent) => {
        if (!this._isDragging) {
            ...
            this._isDragging =
                e.point.dist(this._pointerdownPos) >= clickTolerance;
        }
