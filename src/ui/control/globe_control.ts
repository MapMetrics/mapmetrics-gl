...
export class GlobeControl implements IControl {
    ...
    onAdd(map: Map) {
        ...
        this._globeButton = DOM.create('button', 'mapmetricsgl-ctrl-globe', this._container);
        DOM.create('span', 'mapmetricsgl-ctrl-icon', this._globeButton).setAttribute('aria-hidden', 'true');
    ...
    _updateGlobeIcon = () => {
        this._globeButton.classList.remove('mapmetricsgl-ctrl-globe');
        this._globeButton.classList.remove('mapmetricsgl-ctrl-globe-enabled');
        if (this._map.getProjection()?.type === 'globe') {
            this._globeButton.classList.add('mapmetricsgl-ctrl-globe-enabled');
            this._globeButton.title = this._map._getUIString('GlobeControl.Disable');
        } else {
            this._globeButton.classList.add('mapmetricsgl-ctrl-globe');
