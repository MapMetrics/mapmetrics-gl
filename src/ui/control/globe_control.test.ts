...
describe('GlobeControl', () => {
    ...
    describe('updates control state when Map.setProjection is called', () => {
        ...
        test('default without call to setProjection', () => {
            const button = map.getContainer().querySelector('.mapmetricsgl-ctrl-globe');
            expect(map.style.projection.name).toBe('mercator');
            expect(button.classList.contains('mapmetricsgl-ctrl-globe')).toBeTruthy();
            expect(button.classList.contains('mapmetricsgl-ctrl-globe-enabled')).toBeFalsy();
        });
        ...
        test('setProjection({type: "mercator" -> "globe")', () => {
            ...
            const button = map.getContainer().querySelector('.mapmetricsgl-ctrl-globe-enabled');
            expect(map.style.projection.name).toBe('globe');
            expect(button).not.toBeNull();
            expect(button.classList.contains('mapmetricsgl-ctrl-globe-enabled')).toBeTruthy();
            expect(button.classList.contains('mapmetricsgl-ctrl-globe')).toBeFalsy();
        });
