...
describe('GeolocateControl with no options', () => {
    ...
    test('trigger and then error when tracking user location should get to active error state', async () => {
        ...
        geolocation.sendError({code: 2, message: 'error message'});
        expect(geolocate._watchState).toBe('ACTIVE_ERROR');
        expect(geolocate._geolocateButton.classList.contains('mapmetricsgl-ctrl-geolocate-active-error')).toBeTruthy();
    });
    ...
    test('watching map updates recenter on location with dot', async () => {
        ...
        expect(
            geolocate._userLocationDotMarker._element.classList.contains('mapmetricsgl-user-location-dot-stale')
        ).toBeFalsy();
        ...
        await errorPromise;
        expect(geolocate._userLocationDotMarker._map).toBeTruthy();
        expect(geolocate._userLocationDotMarker._element.classList.contains('mapmetricsgl-user-location-dot-stale')).toBeTruthy();
    });
    ...
    test('Geolocate control should appear only once', async () => {
        ...
        const geolocateUIelem = await geolocateControl._container.getElementsByClassName('mapmetricsgl-ctrl-geolocate');
