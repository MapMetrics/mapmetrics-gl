 ...
 *
 * ## State Diagram
 * ![GeolocateControl state diagram](https://github.com/mapmetrics/mapmetrics-gl-js/assets/3269297/78e720e5-d781-4da8-9803-a7a0e6aaaa9f)
 *
...
export class GeolocateControl extends Evented implements IControl {
    ...
    onAdd(map: Map) {
        this._map = map;
        this._container = DOM.create('div', 'mapmetricsgl-ctrl mapmetricsgl-ctrl-group');
        this._setupUI();
    ...
    _setErrorState() {
        switch (this._watchState) {
            case 'WAITING_ACTIVE':
                ...
                this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-active');
                this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-active-error');
                break;
            case 'ACTIVE_LOCK':
                this._watchState = 'ACTIVE_ERROR';
                this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-active');
                this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-active-error');
                this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-waiting');
                // turn marker grey
                break;
            case 'BACKGROUND':
                this._watchState = 'BACKGROUND_ERROR';
                this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-background');
                this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-background-error');
                this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-waiting');
                // turn marker grey
                break;
            case 'ACTIVE_ERROR':
    ...
    _onSuccess = (position: GeolocationPosition) => {
        ...
        if (this.options.trackUserLocation) {
            ...
            switch (this._watchState) {
                ...
                case 'ACTIVE_ERROR':
                    ...
                    this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-waiting');
                    this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-active-error');
                    this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-active');
                    break;
                case 'BACKGROUND':
                case 'BACKGROUND_ERROR':
                    this._watchState = 'BACKGROUND';
                    this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-waiting');
                    this._geolocateButton.classList.remove('mapmetricsgl-ctrl-geolocate-background-error');
                    this._geolocateButton.classList.add('mapmetricsgl-ctrl-geolocate-background');
                    break;
                default:
                    throw new Error(`Unexpected watchState ${this._watchState}`);
            }
        ...
        if (this.options.showUserLocation) {
            this._dotElement.classList.remove('mapmetricsgl-user-location-dot-stale');
        }
    ...
    _onError = (error: GeolocationPositionError) => {
        ...
        if (this._watchState !== 'OFF' && this.options.showUserLocation) {
            this._dotElement.classList.add('mapmetricsgl-user-location-dot-stale');
        }
    ...
    _setupUI = () => {
        ...
        this._geolocateButton = DOM.create('button', 'mapmetricsgl-ctrl-geolocate', this._container);
        DOM.create('span', 'mapmetricsgl-ctrl-icon', this._geolocateButton).setAttribute('aria-hidden', 'true');
    ...
    _finishSetupUI = (supported: boolean) => {
        ...
        if (this.options.showUserLocation) {
            this._dotElement = DOM.create('div', 'mapmetricsgl-user-location-dot');

            this._userLocationDotMarker = new Marker({element: this._dotElement});

            this._circleElement = DOM.create('div', 'mapmetricsgl-user-location-accuracy-circle');
