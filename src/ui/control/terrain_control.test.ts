...
describe('TerrainControl', () => {
    test('appears in top-right by default', () => {
        ...
        expect(
            map.getContainer().querySelectorAll('.mapmetricsgl-ctrl-top-right .mapmetricsgl-ctrl-terrain')
        ).toHaveLength(1);
    });
    ...
    test('appears in the position specified by the position option', () => {
        ...
        expect(
            map.getContainer().querySelectorAll('.mapmetricsgl-ctrl-bottom-right .mapmetricsgl-ctrl-terrain')
        ).toHaveLength(1);
