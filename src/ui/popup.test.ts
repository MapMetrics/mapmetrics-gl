...
describe('popup', () => {
    ...
    test('Popup.getElement returns a .mapmetricsgl-popup element', () => {
        const map = createMap();
        ...
        expect(popup.getElement().classList.contains('mapmetricsgl-popup')).toBeTruthy();
    });

    test('Popup.addTo adds a .mapmetricsgl-popup element', () => {
        const map = createMap();
        const popup = new Popup()
            .setText('Test')
            .setLngLat([0, 0])
        ...
        expect(popup.isOpen()).toBeTruthy();
        expect(map.getContainer().querySelectorAll('.mapmetricsgl-popup')).toHaveLength(1);
    });
    ...
    test('Popup closes on close button click events', () => {
        ...
        simulate.click(map.getContainer().querySelector('.mapmetricsgl-popup-close-button'));
    ...
    test('Popup has no close button if closeButton option is false', () => {
        ...
        expect(
            popup.getElement().querySelectorAll('.mapmetricsgl-popup-close-button')
        ).toHaveLength(0);
    });
    ...
    test('Popup content can be set via setHTML', () => {
        ...
        expect(popup.getElement().querySelector('.mapmetricsgl-popup-content').innerHTML).toBe('<span>Test</span>');
    });
    ...
    test('Popup content can be set via setDOMContent', () => {
        ...
        expect(popup.getElement().querySelector('.mapmetricsgl-popup-content').firstChild).toBe(content);
    });
    ...
    test('Popup anchors as specified by the anchor option', () => {
        ...
        expect(popup.getElement().classList.contains('mapmetricsgl-popup-anchor-top-left')).toBeTruthy();
    });
    ...
    test.each(anchorCases)(
        ...
        (anchor, point) => {
            ...
            expect(popup.getElement().classList.contains(`mapmetricsgl-popup-anchor-${anchor}`)).toBeTruthy();
        });
    ...
    test('Popup automatically anchors to top if its bottom offset would push it off-screen', () => {
        ...
        expect(popup.getElement().classList.contains('mapmetricsgl-popup-anchor-top')).toBeTruthy();
    });
    ...
    test('Popup can be removed and added again can be closed with click (#5576)', () => {
        ...
        expect(map.getContainer().querySelectorAll('.mapmetricsgl-popup')).toHaveLength(0);
    });
    ...
    test('Popup.addTo is idempotent (#1811)', () => {
        ...
        expect(popup.getElement().querySelector('.mapmetricsgl-popup-content').textContent).toBe('Test');
    });
    ...
    test('Pointer-tracked popup is tagged with right class', () => {
        ...
        ).toContain('mapmetricsgl-popup-track-pointer');
    });
    ...
    test('Pointer-tracked popup can be repositioned with setLngLat', () => {
        ...
        expect(
            popup._container.classList.value
        ).not.toContain('mapmetricsgl-popup-track-pointer');
        expect(
            map._canvasContainer.classList.value
        ).not.toContain('mapmetricsgl-track-pointer');
    });
    ...
    describe('padding', () => {
        ...
        test('trackPointer with padding should not crash and should have track-pointer class', () => {
            ...
            expect(popup.getElement().classList.contains('mapmetricsgl-popup-track-pointer')).toBeTruthy();
            expect(map._canvasContainer.classList.contains('mapmetricsgl-track-pointer')).toBeTruthy();
