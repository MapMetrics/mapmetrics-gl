...
describe('Style#loadJSON', () => {
    ...
    test('creates sources', async () => {
        ...
        await style.once('style.load');
        expect(style.sourceCaches['Mapmetrics'] instanceof SourceCache).toBeTruthy();
    });
    ...
    test('sets up layer event forwarding', () => new Promise<void>(done => {
        ...
        style.on('error', (e) => {
            ...
            expect(e.mapmetrics).toBeTruthy();
            done();
        });

        style.on('style.load', () => {
            style._layers.background.fire(new Event('error', {mapmetrics: true}));
        });
    }));
...
describe('Style#removeSource', () => {
    ...
    async function createStyleAndLoad(): Promise<Style> {
        ...
        style.loadJSON(createStyleJSON({
            'sources': {
                'mapmetrics-source': createGeoJSONSource()
            },
            'layers': [{
                'id': 'mapmetrics-layer',
                'type': 'circle',
                'source': 'mapmetrics-source',
    ...
    test('throws if source is in use', async () => {
        ...
        const promise =  style.once('error');
        style.removeSource('mapmetrics-source');
        const event = await promise;
        expect(event.error.message.includes('"mapmetrics-source"')).toBeTruthy();
        expect(event.error.message.includes('"mapmetrics-layer"')).toBeTruthy();
    });

    test('does not throw if source is not in use', async () => {
        const style = await createStyleAndLoad();
        const promise = style.once('error');
        style.removeLayer('mapmetrics-layer');
        style.removeSource('mapmetrics-source');
...
describe('Style#addLayer', () => {
    ...
    test('reloads source', () => new Promise<void>(done => {
        ...
        style.on('data', (e) => {
            if (e.dataType === 'source' && e.sourceDataType === 'content') {
                style.sourceCaches['Mapmetrics'].reload = () => { done(); };
                style.addLayer(layer);
                style.update({} as EvaluationParameters);
            }
        });
    ...
    test('#3895 reloads source (instead of clearing) if adding this layer with the same type, immediately after removing it', () => new Promise<void>((done) => {
        ...
        style.on('data', (e) => {
            if (e.dataType === 'source' && e.sourceDataType === 'content') {
                style.sourceCaches['Mapmetrics'].reload = () => { done(); };
                style.sourceCaches['Mapmetrics'].clearTiles =  () => { throw new Error('test failed'); };
    ...
    test('clears source (instead of reloading) if adding this layer with a different type, immediately after removing it', () => new Promise<void>((done) => {
        ...
        style.on('data', (e) => {
            if (e.dataType === 'source' && e.sourceDataType === 'content') {
                style.sourceCaches['Mapmetrics'].reload =  () => { throw new Error('test failed'); };
                style.sourceCaches['Mapmetrics'].clearTiles = () => { done(); };
                style.removeLayer('my-layer');
                style.addLayer(layer);
                style.update({} as EvaluationParameters);
            }
...
describe('Style#removeLayer', () => {
    ...
    test('tears down layer event forwarding', () => new Promise<void>((done) => {
        ...
        style.on('style.load', () => {
            ...
            layer.fire(new Event('error', {mapmetrics: true}));
            done();
        });
    }));
...
describe('Style#queryRenderedFeatures', () => {
    ...
    beforeEach(() => new Promise<void>(callback => {
        ...
        function queryMapmetricsFeatures(layers, serializedLayers, getFeatureState, queryGeom, cameraQueryGeom, scale, params) {
            const features = {
                'land': [{
                    type: 'Feature',
        ...
        style.on('style.load', () => {
            style.sourceCaches.mapmetrics.tilesIn = () => {
                return [{
                    tile: {queryRenderedFeatures: queryMapmetricsFeatures} as unknown as Tile,
            ...
            style.sourceCaches.mapmetrics.transform = transform;
    ...
    test('does not query sources not implicated by `layers` parameter', () => {
        style.sourceCaches.mapmetrics.map.queryRenderedFeatures = vi.fn();
        style.queryRenderedFeatures([{x: 0, y: 0} as Point], {layers: ['land--other']}, transform);
        expect(style.sourceCaches.mapmetrics.map.queryRenderedFeatures).not.toHaveBeenCalled();
    });
