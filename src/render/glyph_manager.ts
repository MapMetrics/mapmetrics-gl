...
// FORK PATCH (MAPMETRICS-FORK.md §3 item 11): `@mapbox/tiny-sdf` -> `@mapmetrics/tiny-sdf`.
// A one-line import that is a supply-chain and licence decision. Do not "fix" it back.
//
// KNOWN DEFECT, flagged deliberately rather than hidden: upstream v5.24.0 pins
// `@mapbox/tiny-sdf@^2.1.0` and passes a `lang` option (language-aware glyph rendering).
// `@mapmetrics/tiny-sdf` is still at 0.0.1, forked from an older tiny-sdf that has no `lang`
// -- it neither types it nor reads it. The option is therefore passed and IGNORED, which
// matches the fork's previous behaviour exactly (it never had `lang` at all) but means the
// fork does not get upstream's language-aware rendering. The cast below is what lets that
// compile; it is a deliberate marker, not an accident.
// RESOLUTION NEEDED: republish `@mapmetrics/tiny-sdf` from `@mapbox/tiny-sdf@2.x`, then delete
// the cast. Until then this is a feature gap, not a regression.
...
export class GlyphManager {
    ...
    _createTinySDF(stack: String | false): TinySDF {
        ...
        return new GlyphManager.TinySDF({
            ...
            // See the import note: `@mapmetrics/tiny-sdf@0.0.1` does not declare or read `lang`.
            // Kept so the call site matches upstream and starts working the moment the dependency
            // is republished from @mapbox/tiny-sdf@2.x.
            lang: this.lang
        } as ConstructorParameters<typeof TinySDF>[0] & {lang?: string});
    }
