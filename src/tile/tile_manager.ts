...
export class TileManager extends Evented {
     ...
     *
     * THIS IS A COST GATE, NOT A CORRECTNESS ONE, and that is why losing it is silent. `Map` calls
     * it from `_reloadErroredTiles`, which runs on every v2 map-session credential adoption — i.e.
     * about twice an hour. Without the gate the ENTIRE VIEWPORT is re-fetched on every renewal, on
     * cellular, for a navigation client: pure billed cost with no visible symptom. Do not "simplify"
     * the caller back to an unconditional `reload(true)`.
     *
     * Lived on `SourceCache` before upstream #6635 dissolved that class into `TileManager`; the
     * body is unchanged apart from `this._tiles` becoming `this._inViewTiles.getAllTiles()`.
     * Tests: `tile_manager.test.ts`, 'TileManager#hasErroredTiles'.
     * @returns `true` if at least one held tile is in the `errored` state
     */
    hasErroredTiles(): boolean {
        for (const tile of this._inViewTiles.getAllTiles()) {
            if (tile.state === 'errored') return true;
        }
        return false;
    }
     ...
     */
    refreshTiles(tileIds: ICanonicalTileID[]) {
        for (const id of this._inViewTiles.getAllIds()) {
            const tile = this._inViewTiles.getTileById(id);
     ...
     */
    setDependencies(tileKey: string, namespace: string, dependencies: string[]) {
     ...
     */
    reloadTilesForDependencies(namespaces: string[], keys: string[]) {
        for (const id of this._inViewTiles.getAllIds()) {
            const tile = this._inViewTiles.getTileById(id);
