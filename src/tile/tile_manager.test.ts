...
// Re-homed from `src/source/source_cache.test.ts`, which upstream #6635 deleted along with
// `SourceCache` itself. These are the ONLY automated evidence for MAPMETRICS-FORK.md §3 item 5 —
// the gate that stops a full-viewport re-download on every v2 credential renewal. If a future
// re-vendor cannot find a home for them, that is a signal to stop, not to drop them.
describe('TileManager#hasErroredTiles', () => {
    test('false when the tile manager is empty', () => {
        const tileManager = createTileManager({});
        expect(tileManager.hasErroredTiles()).toBe(false);
    });

    test('false when every held tile loaded', () => {
        const tileManager = createTileManager({});
        const tile = tileManager.addTile(new OverscaledTileID(0, 0, 0, 0, 0));
        tile.state = 'loaded';
        expect(tileManager.hasErroredTiles()).toBe(false);
    });
    ...
    test('true when any held tile errored', () => {
        const tileManager = createTileManager({});
        tileManager.addTile(new OverscaledTileID(0, 0, 0, 0, 0)).state = 'loaded';
        tileManager.addTile(new OverscaledTileID(1, 0, 1, 0, 0)).state = 'errored';
        expect(tileManager.hasErroredTiles()).toBe(true);
    });
});
