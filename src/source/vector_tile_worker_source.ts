...
export class VectorTileWorkerSource implements WorkerSource {
    ...
    _getExpiryData({expires, cacheControl, etag, mapSessionHeaders}: ExpiryData): ExpiryData {
        ...
        // arrives on a tile response and the tile is fetched here, on a worker thread. This is the
        // ONLY channel by which the credential is ever replaced.
        //
        // Pre-v5.24.0 the fork patched the `loadTile` and `reloadTile` halves separately. Upstream
        // unified them into this one helper for `etag`, which is the same response-header ->
        // main-thread problem, so both halves are now covered here -- plus the etag-unmodified
        // path, which the two-half version missed. Keep it on this line, next to `etag`.
        if (mapSessionHeaders) data.mapSessionHeaders = mapSessionHeaders;
        return data;
    }
