...
const outputFile = production ? 'dist/mapmetrics-gl.js' : 'dist/mapmetrics-gl-dev.js';

const config: RollupOptions[] = [{
    // Rollup will use code splitting to bundle GL JS into three "chunks":
    // - staging/mapmetricsgl/index.js: the main module, plus all its dependencies not shared by the worker module
    // - staging/mapmetricsgl/worker.js: the worker module, plus all dependencies not shared by the main module
    // - staging/mapmetricsgl/shared.js: the set of modules that are dependencies of both the main module and the worker module
    //
    ...
    // rollup/mapmetricsgl.js for details.
